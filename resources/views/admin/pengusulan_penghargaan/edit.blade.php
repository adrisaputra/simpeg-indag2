...
<section class="content-header">
	...
	<div class="box">
		...
		<form action="{{ url('/'.Request::segment(1).'/edit/'.$pengusulan_penghargaan->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
		{{ csrf_field() }}
				...
				<div class="col-lg-12">
						...
						<div class="col-sm-10">
							...
							<select class="form-control select2" name="nama_pegawai">
								...
								@foreach($pegawai as $v)
                                        	<option value="{{ $v->nip }}" @if($pengusulan_penghargaan->nip==$v->nip) selected @endif> {{ $v->nama_pegawai }}</option>
								@endforeach
                                   </select>
						</div>
					</div>
					...
					<div class="form-group @if ($errors->has('jenis')) has-error @endif">
						...
						<div class="col-sm-10">
							...
							<input type="text" class="form-control" placeholder="Jenis" name="jenis" value="{{ $pengusulan_penghargaan->jenis }}" >
						</div>
					</div>
					...
					<div class="form-group @if ($errors->has('syarat')) has-error @endif">
						...
						<div class="col-sm-10">
							...
							<textarea class="form-control" placeholder="Syarat" name="syarat">{{ $pengusulan_penghargaan->syarat }}</textarea>
						</div>
					</div>
