...
<section class="content-header">
	...
	<div class="box">
		...
		<form action="{{ url('/riwayat_anak/'.$pegawai[0]->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
		@csrf
				...
				<center><p style="font-size:20px">DATA ANAK</p></center>
					...
					<div class="form-group @if ($errors->has('nama_anak')) has-error @endif">
						...
						<div class="col-sm-10">
							...
							<input type="text" class="form-control" placeholder="Nama Anak" name="nama_anak" value="{{ old('nama_anak') }}" >
						</div>
					</div>
					...
					<div class="form-group @if ($errors->has('status')) has-error @endif">
						...
						<div class="col-sm-10">
							...
							<select class="form-control select2" name="status">
                                        ...
                                        <option value="Kandung" @if(old('status')=="Kandung") selected @endif> Kandung</option>
                                        <option value="Tiri" @if(old('status')=="Tiri") selected @endif> Tiri</option>
                                        <option value="Angkat" @if(old('status')=="Angkat") selected @endif> Angkat</option>
					...
					<div class="form-group @if ($errors->has('pendidikan')) has-error @endif">
						...
						<div class="col-sm-10">
							...
							<input type="text" class="form-control" placeholder="Pendidikan" name="pendidikan" value="{{ old('pendidikan') }}" >
						</div>
					</div>
					
					<div class="form-group @if ($errors->has('akta_kelahiran')) has-error @endif">
						<label class="col-sm-2 control-label">{{ __('Akta Kelahiran') }}</label>
						<div class="col-sm-10">
