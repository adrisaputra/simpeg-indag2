...
<div class="content-wrapper">
	...
	<div class="box">   
		<div class="box-header with-border">
			...
			<div class="table-responsive box-body">
				...
				<table class="table table-bordered">
					...
					@foreach($pengumuman_ujian_dinas as $v)
					<tr>
						<td>{{ ($pengumuman_ujian_dinas ->currentpage()-1) * $pengumuman_ujian_dinas ->perpage() + $loop->index + 1 }}</td>
						<td>{{ $v->lokasi }}</td>
						<td>{{ $v->syarat }}</td>
		...
		<div class="box-footer">
			<!-- PAGINATION -->
			<div class="float-right">{{ $pengumuman_ujian_dinas->appends(Request::only('search'))->links() }}</div>
		</div>
	</div>
	</section>
</div>
