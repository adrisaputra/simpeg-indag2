...
<div class="content-wrapper">
	...
	<div class="box">   
		<div class="box-header with-border">
			<div class="box-tools pull-left">
				<div style="padding-top:10px">
					<a href="{{ url('/riwayat_diklat/create/'.$pegawai[0]->id) }}" class="btn btn-success btn-flat" title="Tambah Data">Tambah</a>
					<a href="{{ url('/riwayat_diklat/'.$pegawai[0]->id) }}" class="btn btn-warning btn-flat" title="Refresh halaman">Refresh</a>    
					<a href="{{ url('/pegawai') }}" class="btn btn-danger btn-flat" title="Refresh halaman">Kembali</a>    
				</div>
			</div>
			<div class="box-tools pull-right">
				<div class="form-inline">
					<form action="{{ url('/riwayat_diklat/search/'.$pegawai[0]->id) }}" method="GET">
		...
		<div class="box-footer">
			<!-- PAGINATION -->
			<div class="float-right">{{ $riwayat_diklat->appends(Request::only('search'))->links() }}</div>
		</div>
	</div>
	</section>
</div>
